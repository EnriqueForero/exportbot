...
def feedback(entrada: FeedbackEntrada, request: Request) -> dict:
    ...
    request.app.state.telemetria.log_feedback(entrada.chat_id, entrada.util, entrada.comentario)
    return {"ok": True}
...
def evento(entrada: EventoEntrada, request: Request) -> dict:
    ...
    request.app.state.telemetria.log_evento(entrada.evento, entrada.detalles, entrada.session_id)
    return {"ok": True}
